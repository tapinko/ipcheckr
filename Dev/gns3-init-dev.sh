...
CONNECTOR_URL="${LAUNCHER_URL}/ipcheckr-gns3-connector.sh"
INSTALLER_URL="${LAUNCHER_URL}/scripts/gns3-install.sh"
GNS3_UNIT_URL="${LAUNCHER_URL}/systemd/gns3@.service"
SERVICE_URL="${LAUNCHER_URL}/systemd/ipcheckr-gns3.service"
SOCKET_URL="${LAUNCHER_URL}/systemd/ipcheckr-gns3.socket"
DEV_CONNECTOR_ENV_URL="${BASE_URL}/Dev/connector-dev.env"

CONNECTOR_TMP="/tmp/ipcheckr-gns3-connector.sh"
INSTALLER_TMP="/tmp/gns3-install.sh"
CONNECTOR_ENV_TMP="/tmp/ipcheckr-connector-dev.env"
...
CONNECTOR_ENV_PATH="/etc/ipcheckr/gns3/connector.env"
...
sudo mkdir -p "$(dirname "$CONNECTOR_ENV_PATH")"
curl -fsSL -o "$CONNECTOR_ENV_TMP" "$DEV_CONNECTOR_ENV_URL"
sudo install -m 0640 "$CONNECTOR_ENV_TMP" "$CONNECTOR_ENV_PATH"
sudo chown root:ipcheckr "$CONNECTOR_ENV_PATH" 2>/dev/null || true
