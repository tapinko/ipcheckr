...
import Gns3DataGridWithSearch from "../../components/ui/Gns3DataGridWithSearch"
...
const TeacherGns3AllSessions = () => {
  ...
  const startSessionMutation = useMutation({
    ...
    onSuccess: (_res, variables) => {
      ...
      setAlert({ severity: "success", message: `${t(TranslationKey.TEACHER_GNS3_ALL_SESSIONS_START, { value: variables.username })}` })
    },
  ...
  const stopSessionMutation = useMutation({
    ...
    onSuccess: (_res, variables) => {
      queryClient.invalidateQueries({ queryKey: ["gns3-sessions"] })
      setAlert({ severity: "success", message: `${t(TranslationKey.TEACHER_GNS3_ALL_SESSIONS_STOP, { value: variables.username })}` })
    },
    onError: (error, variables) => {
      const message = getApiMessage(error)
      setAlert({
  ...
  const extendSessionMutation = useMutation({
    ...
    onSuccess: (_res, variables) => {
      ...
      setAlert({
        severity: "success",
        message: `${t(TranslationKey.TEACHER_GNS3_ALL_SESSIONS_EXTEND, { value: variables.username, minutes: variables.minutes })}`,
      })
    },
    onError: (error, variables) => {
      const message = getApiMessage(error)
  ...
  const classFilter = useMemo(() => {
    ...
    return {
      label: t(TranslationKey.TEACHER_GNS3_ALL_SESSIONS_CLASS_FILTER),
      value: classFilterValue,
      setValue: setClassFilterValue,
      options: [
        { value: "", label: t(TranslationKey.TEACHER_GNS3_ALL_SESSIONS_ALL_CLASSES) },
...
export default TeacherGns3AllSessions