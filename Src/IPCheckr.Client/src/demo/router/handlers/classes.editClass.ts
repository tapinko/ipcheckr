...
export const editClassHandler: DemoEndpointHandler = async ({ config, path, method }) => {
  ...
  const body = parseBody(config) as { id?: number; classname?: string; teachers?: number[]; students?: number[] } | null
  const id = Number(body?.id ?? 0)
  if (!id) return demoResponse(config, { messageEn: "Class not found" }, 400)

  const success = await editClass(id, body?.classname || null, body?.teachers || null, body?.students ?? null)
